...
public class SparkApp {
 ...
 |-- div5tailnum: integer (nullable = true)
  ...
  public static JavaRDD<Row> Q3A(SparkSession spark, String dataFile) {
    ...
    // TODO your code here
    d.createOrReplaceTempView("flights");
    Dataset<Row> row = spark.sql("SELECT DISTINCT destcityname FROM flights " +
            "WHERE origincityname = 'Seattle, WA'");
    JavaRDD<Row> rdd = row.javaRDD();
  ...
  public static JavaRDD<Row> Q3B(SparkSession spark, String dataFile) {
    ...
    // TODO your code here
    Dataset<Row> row = d.select(d.col("origincityname"), d.col("month"))
                        .filter(d.col("cancelled").equalTo(0))
  ...
  public static JavaRDD<Row> Q3C(SparkSession spark, String dataFile) {
    ...
    // TODO your code here
    Dataset<Row> row = d.select(d.col("origincityname"), d.col("depdelay"))
                          .filter(d.col("depdelay").isNotNull().and(d.col("cancelled").equalTo(0)))
                          .groupBy(d.col("origincityname"))
                          .avg("depdelay");
    Dataset<Row> result = row.select(row.col("origincityname"), row.col("avg(depdelay)"));
    JavaRDD<Row> rdd = result.javaRDD();
    return rdd;
  }
